...
- **A guard must fail loud, never quiet.** Work out what your check does when the *check itself* fails — a missing credential, an unauthenticated CLI, an empty response. If that path silently reports "fine", the guard is worse than none. Prefer a signal the checker already reads over a new dependency that can fail in a way that looks like success.
- **Observability must never decide a gate.** Tracing, metrics and dataset recording are side effects: if Langfuse, PostHog or Sentry fails, log one line and carry on. A run whose work succeeded must not exit non-zero because it could not write telemetry.
- **Pin behaviour, not source text.** A test that matches the script's exact characters breaks on a correct fix and passes a wrong one. Where the behaviour is executable — a shell parameter default, a rendered class, a built URL — run it and assert the result.
